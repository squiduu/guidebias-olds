...
def run_finetune(data_args: DataArguments, model_args: ModelArguments, train_args: TrainingArguments, logger: Logger):
    ...
    with open(file="./data/stereotype/stereotype_words.json", mode="r") as ster_fp:
    ...
    WIKI_WORDS = clean_words(words=WIKI_WORDS, tokenizer=tokenizer)
    ...
    for epoch in range(1, int(train_args.num_epochs) + 1):
        ...
        for iter, batch_idx in enumerate(dataloader):
            ...
            with amp.autocast_mode.autocast():
                with torch.no_grad():
                    #
                    male_probs, male_stereotype_probs = get_probs(
                        model=model,
                        inputs=male_inputs_batch,
                        mask_idx=male_mask_idx_batch,
                        stereotype_ids=STEREOTYPE_IDS,
                    )
                    female_probs, female_stereotype_probs = get_probs(
                        model=model,
                    ...
                    #
                    male_stereotype_ids, female_stereotype_ids = get_unmasked_ids(
                    ...
                    #
                    male_switched_sents, female_switched_sents = get_switched_sents(
                    ...
                    masked_inputs, labels = prepare_masked_inputs_and_labels(
                        male_sents=male_switched_sents,
                        female_sents=female_switched_sents,
                        mask_idx=torch.concat([male_mask_idx_batch, female_mask_idx_batch]),
                        tokenizer=tokenizer,
                        device=device,
                    )
