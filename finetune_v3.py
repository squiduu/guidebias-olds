        ...
        for iter, batch_idx in enumerate(dataloader):
            ...
            with amp.autocast_mode.autocast():
                with torch.no_grad():
                    ...
                    #
                    male_unmasked_ids, female_unmasked_ids = get_unmasked_ids(
                        male_inputs=male_inputs_batch,
                        male_mask_idx=male_mask_idx_batch,
                    ...
                    #
                    switched_sents = get_switched_sents(
                        male_ids=male_unmasked_ids, female_ids=female_unmasked_ids, tokenizer=tokenizer
                    )

                    #
                    masked_inputs, labels = prepare_masked_inputs_and_labels(
                        sents=switched_sents, tokenizer=tokenizer, model_args=model_args
                    )
